            ...
            <div class="board__data">
                ...
                <div class="board__list">
                    <?php foreach($posts as $post) { ?>
                        <div class="post p5"><span class="post__rank"><?= $post->idx ?></span><a href="/boardDetail/<?= $post->idx ?>" class="post__title"><?= $post->title ?></a><span class="post__date"><?= $post->date ?></span><span class="post__like"><svg viewBox="0 0 24 24"><path d="M12 21s-7.5-4.6-10-9.2C.3 8.5 1.9 5 5.2 5c2 0 3.3 1.1 4 2.2C9.8 6.1 11.2 5 13.1 5c3.3 0 4.9 3.5 3.2 6.8C19.5 16.4 12 21 12 21z"/></svg>좋아용</span></div>
                    <?php } ?>
                </div>
                ...
                <div class="pager">
                    <?php if ($page > 1) { ?>
                        <a href="?page=<?= $page - 1 ?>" class="prevBtn">이전</a>
                    <?php } ?>
                    <?php for ($i = 1; $i <= $maxPage; $i++) { ?>
                        <a href="?page=<?= $i ?>" class="<?= $i == $page ? "active" : "" ?>"><?= $i ?></a>
                    <?php } ?>
                    <?php if ($page < $maxPage) { ?>
                        <a href="?page=<?= $page + 1 ?>" class="nextBtn">다음</a>
                    <?php } ?>
                </div>
